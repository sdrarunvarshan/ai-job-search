...
- **Community Typst CV + cover-letter pack** - `typst-modern/` is a bring-your-own pair for discussion #351 (2-page CV, 1-page cover letter, single column). Stock templates stay LaTeX. Register with `/add-template`; bundled `moderncv` / `cover.cls` are unchanged. After both files are active, `/apply` runs `typst compile`. **Not an upstream PR** — Mads asked that Typst stay out of the bundled tree and be announced in discussion #78.
- **India market + remote portal skills** (from @ashutoshgh / ai-job-search-india) - 10 India CLIs (Apna, Hirist, IIMJobs, Instahyre, Indeed India, Randstad India, Cutshort, AIJobs, Foundit, ProtocolJobs) and 5 remote-board CLIs (Himalayas, RemoteOK, We Work Remotely, Y Combinator, Otta). Each sends an identifying User-Agent. **Heads-up:** `indeed-india-search` `detail` fetches a path Indeed's `robots.txt` disallows — documented in the skill; personal use only. Foundit is **detail-only** (search is robots-disallowed / bot-blocked).
- **Universal Edition runtime pointers** (from @waleedtarbosh) - `AGENTS.md` remains the index; `OPENCODE.md`, `GEMINI.md`, and `CODEX.md` are thin pointers so OpenCode / Gemini / Codex share the same `.claude/commands/` workflows. Identical `job-search` router skills live under `.agents/skills/`, `.claude/skills/`, and `.opencode/skills/`. `/scrape` skips the router folder (it is not a portal CLI). Candidate profile stays in `CLAUDE.md` so `/setup` is unchanged.

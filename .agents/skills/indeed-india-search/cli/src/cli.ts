#!/usr/bin/env bun
// Self-contained CLI for searching jobs on Indeed India (https://in.indeed.com).
// No external CLI framework, zero runtime dependencies — runs anywhere `bun` is.
//
// Personal use only. Indeed's robots.txt disallows /viewjob (and other listing
// paths) and its terms restrict automated access. Keep volume low, do not use it
...
const HELP = `indeed-india-cli — search jobs on Indeed India (https://in.indeed.com)
...
SEARCH FLAGS
  ...
  --location, -l <text>   City / state, e.g. "Chennai", "Bengaluru, Karnataka".
                          Omit to search all of India.
  --jobage <days>         Posted within N days: 1, 3, 7, 14. Default: all.
...
Personal use only — Indeed's robots.txt disallows /viewjob; keep volume low.
`
  ...
  if (cmd === "search") {
    ...
    if (!query && !location) {
      process.stderr.write(
        JSON.stringify({
          error: 'provide at least --query/-q or --location/-l (e.g. -q "accountant" -l "Chennai")',
          code: "NO_CRITERIA",
        }) + "\n",
      )
      return 1
